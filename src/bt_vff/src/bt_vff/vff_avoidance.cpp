...
#include"bt_vff/vff_avoidance.hpp"
...
namespace bt_vff
{
    ...
    vff_avoidance::vff_avoidance(const std::string &xml_pkg_name, const BT::NodeConfiguration &conf)
    :BT::ActionNodeBase(xml_pkg_name, conf)
    {
        config().blackboard->get("node_name", node_);
        scan_sub = node_->create_subscription<sensor_msgs::msg::LaserScan>("input_scan", rclcpp::SensorDataQoS(),
                                                                           [this](sensor_msgs::msg::LaserScan::UniquePtr msg_)
                                                                           {latest_scan = std::move(msg_);});
        twist_pub = node_->create_publisher<geometry_msgs::msg::Twist>("vel_output", 100);
        marker_array_pub = node_->create_publisher<visualization_msgs::msg::MarkerArray>("output_vff_debug", 100);
    }

    void vff_avoidance::halt()
    {
        geometry_msgs::msg::Twist stop_msg;
        twist_pub->publish(stop_msg);
    }

    BT::NodeStatus vff_avoidance::tick()
    {
        if(latest_scan==nullptr or node_->now()-latest_scan->header.stamp>1s)
        {
            return BT::NodeStatus::FAILURE;
        }

        const vff_vector &vffvec = get_vff(*latest_scan);
    ...
    vff_vector vff_avoidance::get_vff(const sensor_msgs::msg::LaserScan &laser_msg)
    {
    ...
    vff_avoidance::get_vff_debug(const vff_vector &vectors)
    {
    ...
    visualization_msgs::msg::Marker vff_avoidance::make_marker(const std::vector<float> &vector_, vff_colour colour_)
    {
...
{
    factory.registerNodeType<bt_vff::vff_avoidance>("vff_avoidance");
}